...
public function _displayAdvertising()
{
	...
	public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
	{
		...
		if (!empty($is_intersected))
		{
			...
			$this->respMenu .= '<div class="dropdown">';
			$this->respMenu .= '<a class="'.$class_ln.'" href="'.$cat_link.'" >';
			if ($type_icon == 1 && $icon != '')
				$this->respMenu .= '<i class="'.$icon.'"></i>';
			elseif ($type_icon == 0 && $icon != '')
				$this->respMenu .= '<img src="'.$icon_path.$icon.'" alt=""/>';
			$this->respMenu .= ''.$category->name.'</a>';
		...
		if (isset($children) && count($children))
		{
			$this->respMenu .= '<a href="#" class="dropdown-toggle dropdown-toggle-split" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><i class="mdi mdi-chevron-down"></i><span class="sr-only">Toggle Dropdown</span></a>';
			$this->respMenu .= '<ul class="dropdown-menu">';
