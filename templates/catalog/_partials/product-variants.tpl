    ...
    <div class="form-group row no-gutters">
      ...
      {if $group.group_type == 'select'}
        ...
        {block name='product_variants_select'}
          <select data-product-attribute="{$id_attribute_group}" name="group[$id_attribute_group}]" id="group_{$id_attribute_group}" class="col-6 custom-select px-2">
      ...
      {else if $group.group_type == 'color'}
        ...
        {block name='product_variants_color'}
          <ul id="group_{$id_attribute_group}" class="list-inline my-0">
            {foreach from=$group.attributes key=id_attribute item=group_attribute}
              <li class="input-container list-inline-item">
                <input class="input-color" type="radio" data-product-attribute="{$id_attribute_group}" name="group[{$id_attribute_group}]" value="{$id_attribute}" {if $group_attribute.selected} checked="checked"{/if}>
                <span {if $group_attribute.html_color_code} style="background-color: {$group_attribute.html_color_code}" {/if} {if $group_attribute.texture} style="background-image: url({$group_attribute.texture})" {/if} class="color">
